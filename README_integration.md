...
The response uses the generation input shape, but is not an exact replay: `prompt` is the original submitted string, before styles and moderation, while parameters include defaults and style overrides. Legacy requests without a recorded original omit `prompt`; the effective worker prompt is never used as a fallback. Clients must retain the original style/submission to reproduce a styled request. Options that only steer submission (`dry_run`, `allow_downgrade`, `replacement_filter`, `style`) are not returned. Source images come back as uploaded storage references, not submitted base64 data. Responses use `Cache-Control: private, no-store`.
