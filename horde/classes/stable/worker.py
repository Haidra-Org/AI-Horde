...
class WorkerExtended(Worker):
    ...
    def can_generate(self, waiting_prompt):
        ...
        if waiting_prompt.source_image and check_bridge_capability("img2img", self.bridge_agent):
        ...
        if waiting_prompt.source_processing not in ['inpainting', 'outpainting'] and self.get_model_names() == ["stable_diffusion_inpainting"]:
            return [False, 'models']
        #logger.warning(datetime.utcnow())
        if waiting_prompt.source_processing != 'img2img' and check_bridge_capability("img2img", self.bridge_agent):
            return [False, 'painting']
        ...
        #logger.warning(datetime.utcnow())
        if len(waiting_prompt.gen_payload.get('post_processing', [])) >= 1 and check_bridge_capability("post-processing", self.bridge_agent):
            return [False, 'bridge_version']
        if "CodeFormers" in waiting_prompt.gen_payload.get('post_processing', []) and check_bridge_capability("CodeFormers", self.bridge_agent):
            return [False, 'bridge_version']
        #logger.warning(datetime.utcnow())
