...
class ImageAsyncGenerate(GenerateTemplate):
    ...
    def validate(self):
        ...
        if "hires_fix" in self.params and any(model_name in ["SDXL 1.0"] for model_name in self.args.models):
...
class ImageAsyncStatus(Resource):
    ...
    def get(self, id = ''):
        ...
        if not wp:
            raise e.RequestNotFound(id,request_type="Image Waiting Prompt (Status)",client_agent=self.args["Client-Agent"],ipaddr=request.remote_addr)
    ...
    def delete(self, id = ''):
        ...
        wp = database.get_wp_by_id(id)
        if not wp:
            raise e.RequestNotFound(id,request_type="Image Waiting Prompt (Delete)",client_agent=self.args["Client-Agent"],ipaddr=request.remote_addr)
        wp_status = wp.get_status(
            request_avg=database.get_request_avg("image"),
            has_valid_workers=database.wp_has_valid_workers(wp),
            wp_queue_stats=database.get_wp_queue_stats(wp),
...
class Aesthetics(Resource):
    ...
    def post(self, id):
        ...
        wp = database.get_wp_by_id(id)
        if not wp:
            raise e.RequestNotFound(id,request_type="Image Waiting Prompt (Aesthetics)",client_agent=self.args["Client-Agent"])
